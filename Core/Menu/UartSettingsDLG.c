						...
						case WM_NOTIFICATION_RELEASED:
							if (pdTRUE != bluetoothConnected) {
								...
								* different than 0 an error occurred*/
								 ...
								 *  different than 0 an error occurred*/
								if (USART_ReceiveString (USART1, UART1_RXq,
								...
								if (currentBTUARTSettings.Parity !=  lastBTUARTSettings.Parity) {
									...
									switch (currentBTUARTSettings.Parity) {
										case 0:
											if (USART_SendString (USART1, UART1_TXq, 6,
																 "ATM0\r\0")) {
												/* no parity*/
												configErr++;
											}
									...
									/* try to receive string of data :: if the returned value is
									 * different than 0 an error occurred*/
									if (USART_ReceiveString (USART1,UART1_RXq, sizeof (rxBuffer),
															 rxBuffer)) {
										configErr++;
									}
									else {
								...
								if (currentBTUARTSettings.BaudRate != lastBTUARTSettings.BaudRate) {
									...
									/* try to send string of data :: if the returned value is
									 * different than 0 an error occurred*/
									if (USART_SendString (USART1,UART1_TXq, sizeof (ATCommandArray),
														  ATCommandArray)) {
										configErr++;
									}
									/* try to receive string of data :: if the returned value is
...
/**
  ...
  */
int8_t USART_SendString (USART_TypeDef* USARTx, xQueueHandle UARTx_TXq, uint8_t length,
						const char* Data) {
...
/**
  ...
  * @retval error messages
  */
int8_t USART_ReceiveString (USART_TypeDef* USARTx, xQueueHandle UARTx_RXq, uint8_t length,
							char* bufferRx) {
	uint16_t i = 0;
